...
const Dashboard = () => {
  ...
  const [allItems, setAllItems] = useState(0); // Initialize with 0 and update from backend
  ...
  useEffect(() => {
    const fetchInitialCounts = async () => {
      try {
        ...
        const itemCountResponse = await axios.get('http://localhost:8080/api/items/count');
        setAllItems(itemCountResponse.data);
      } catch (error) {
        console.error('Error fetching shipment status counts, low stock items, or item count:', error);
      }
    };
  ...
  return (
    <Box sx={{ display: 'flex', flexDirection: 'column', minHeight: '100vh', backgroundColor: '#F5DEB3' }}>
      <Sidebar1 />
